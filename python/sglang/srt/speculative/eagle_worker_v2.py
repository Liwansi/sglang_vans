...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def forward_batch_generation(
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            is_last_pp = self.draft_worker.draft_worker.pp_group.is_last_rank

            if not is_last_pp:
                # Non-last PP rank: just run target forward.
                # The verify input (draft tokens from previous step) is
                # already prepared on rank 0 and flows through the PP pipeline.
                return self.target_worker.forward_batch_generation(
                    batch, pp_proxy_tensors=pp_proxy_tensors
                )

            # Last PP rank: full MTP decode
