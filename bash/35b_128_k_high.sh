...
python3 -m sglang.launch_server \
        ...
        --device npu \
        --tp-size 2 \
        --chunked-prefill-size -1 --max-prefill-tokens 128000 \
        --disable-radix-cache --base-gpu-id 12 \
        --trust-remote-code \
        --host 127.0.0.1 --max-running-requests 3 --max-mamba-cache-size 10 \
        --mem-fraction-static 0.64 \
        --enable-prefill-delayer \
        --prefill-delayer-max-delay-passes 20 \
	--port 6003 \
        --cuda-graph-bs 2 3 \
        --enable-multimodal \
